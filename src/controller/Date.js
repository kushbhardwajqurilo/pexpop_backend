...
exports.addDate = async (req, res) => {
    ...
    try {
        const dateData = await DateRecordModel.create({ date, costing_inr })
        if (dateData) {
            res.json({
                status: "success",
...
exports.getAllDate = async (req, res) => {
    try {
        const dateRecords = await DateRecordModel.find({}).sort({ date: -1 })
        if (dateRecords.length > 0) {
            const updatedDateRecords = dateRecords.map((data) => {
                ...
                const total_profit = total_revenue - data.costing_inr;
                const total_agent_commission = data.entries.reduce((total, entry) => total + entry.total_sale_usd * entry.agent_commission, 0);
...
exports.editDate = async (req, res) => {
    ...
    try {
        const dateData = await DateRecordModel.findByIdAndUpdate(id, { costing_inr })
        if (dateData) {
            res.json({
                status: "success",
...
exports.searchDate = async (req, res) => {
    ...
    try {
        ...
        const allEnteries = await DateRecordModel.find({ date: { $gte: start, $lte: end } })
...
exports.getEntryByDateRange = async (req, res) => {
    ...
    try {
        ...
        const dateRecords = await DateRecordModel.find({ date: { $gte: data.from, $lte: data.to } }).sort({ date: -1 })
        ...
        if (dateRecords.length > 0) {
            ...
            const updatedDateRecords = dateRecords.map((data) => {
                ...
                const total_agent_commission = parseInt(data.entries.reduce((total, entry) => total + entry.total_sale_usd * entry.agent_commission, 0));
                const combined = {
                    total_sale_usd,
                    total_sale_inr,
                    total_revenue,
...
exports.deleteDate = async (req, res) => {
    const id = req.params.id
    try {
        const dateData = await DateRecordModel.deleteOne({ _id: id })
        if (dateData) {
            res.json({
                status: "success",
...
exports.getDateByDateRangeFromTo = async (req, res) => {
    ...
    try {
        ...
        const allEnteries = await DateRecordModel.find({ createdAt: { $gte: data.from, $lte: data.to } })
