...
exports.addUserEntry = async (req, res) => {
  try {
    ...
    const dateRecord = await DateRecordModel.findOne({
      date: { $gte: from, $lte: to },
    });
    if (dateRecord) {
      const total_sale_inr =
        data.sale1 * data.rate1 +
        data.sale2 * data.rate2 +
        data.sale3 * data.rate3;
...
exports.updateUserEntry = async (req, res) => {
  ...
  try {
    ...
    const Index = dateRecord.entries.findIndex(
...
exports.searchUserEntry = async (req, res) => {
  ...
  try {
    const ress = await UserEntryModel.find({
